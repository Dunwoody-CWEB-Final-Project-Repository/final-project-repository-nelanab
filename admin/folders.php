    ...
    <body>
        <div id="container">
            ...
            <table class="table table-hover table-striped table-dark" id="folders" >
                ...
                <tbody>
                <?PHP 
                        $getFolders = "SELECT folderID, username, name, f.imageID, i.image FROM folders f JOIN users u ON f.userID = u.userID JOIN images i ON i.imageID = f.imageID ORDER BY username LIMIT :from_record_num, :records_per_page ;";
                        $stmt = $db->prepare($getFolders);
                        ...
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
                            extract($row);?>
                            <tr>
                                <th scope="row"><?PHP echo $folderID ?></th>
                                ...
                                <td><a href ="#" id='delete' class="btn btn-outline-danger" style='cursor: pointer;' onclick='delete_folder(<?PHP echo $folderID ?>);' class='btn btn-danger delete-object'>delete</a></td>
                            </tr>
                    <?PHP } $stmt->closeCursor();?>
                    <tr>
                            <?php 
                                $query = "SELECT COUNT(*) as total_rows FROM folders";
